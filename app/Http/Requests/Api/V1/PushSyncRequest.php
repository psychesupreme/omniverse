...
class PushSyncRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'data.tasks' => 'nullable|array',
            'data.tasks.*.id' => 'required',
            'data.tasks.*.status' => 'required|string',
            'data.tasks.*.completion_notes' => 'nullable|string',
            'data.tasks.*.last_updated_at' => 'required|date',
            'data.tasks.*.evidence_photos' => 'nullable|array',
            'data.orders' => 'nullable|array',
            'data.orders.*.id' => 'required',
            'data.orders.*.order_number' => 'required|string',
            'data.orders.*.total_amount' => 'required|numeric',
            'data.orders.*.status' => 'required|string',
            'data.orders.*.notes' => 'nullable|string',
            'data.orders.*.placed_at' => 'required|date',
            'data.orders.*.items' => 'required|array',
            'data.orders.*.items.*.product_id' => 'required',
            'data.orders.*.items.*.quantity' => 'required|integer|min:1',
            'data.orders.*.items.*.unit_price' => 'required|numeric',
            'data.orders.*.items.*.subtotal' => 'required|numeric',
        ];
    }
}
